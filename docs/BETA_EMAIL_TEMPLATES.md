# Pipeline 360 UVIP Beta Email Templates

## 1. Application Confirmation (Auto-sent)
> Already configured in the system - sent automatically on application submit

---

## 2. Manual Acceptance Email

**Subject:** Welcome to Pipeline 360 UVIP Beta!

```
Hi [Name],

Great news - you're in! Welcome to the Pipeline 360 UVIP beta program.

Let's get you started with a quick 15-minute demo call. I'll show you the platform, answer questions, and get you access.

When works for you this week?
[Your calendar link]

Or just reply with your availability and I'll send a meeting invite.

What to expect on the call:
- Quick platform demo
- Your login credentials
- Answer any questions
- Set up your first call/clip

Looking forward to it!

Best,
Tim

P.S. You're one of only 10 teams in this beta - excited to have you!
```

---

## 3. Welcome Email (After Onboarding Call)

**Subject:** Your Pipeline 360 UVIP Access

```
Hi [Name],

Great talking to you today! As promised, here's your access:

LOGIN DETAILS:
URL: https://mortgage-crm-production-7a9a.up.railway.app
Email: [their-email]
Temporary Password: [temp-password]
(You'll be prompted to change this on first login)

GETTING STARTED:
1. Log in and complete the onboarding tour
2. Record your first call
3. Create a video clip (try the "Rate Lock Explanation" template)
4. View your analytics after a few calls

THIS WEEK'S CHALLENGE:
- Record 3 calls
- Create 1 video clip
- Share the clip with a borrower

NEED HELP?
- In-app help panel: Click "?" button
- Email: Just reply to this
- Check-in call: [Scheduled date/time]

Thanks for being part of the beta!

Best,
Tim
```

---

## 4. Day 3 Check-In Email

**Subject:** Day 3 - How's it going, [Name]?

```
Hi [Name],

Quick check-in! You've had access for 3 days now.

Quick questions:
- Have you recorded any calls/clips yet?
- Anything confusing or broken?
- What's working well?

Just reply to this email - I read every response.

Best,
Tim
```

---

## 5. Weekly Check-In Email

**Subject:** Week [X] Check-In - [Name]

```
Hi [Name],

Hope you're having a great week!

Quick check-in on your UVIP experience:

THIS WEEK'S QUESTIONS:
1. What features did you use most?
2. Anything confusing or frustrating?
3. What's working really well?
4. Any features you wish existed?

YOUR STATS THIS WEEK:
- Calls recorded: [X]
- Clips created: [X]
- Avg engagement score: [X.X]

Just reply to this email with your thoughts!

Or if you prefer to chat:
[Calendar link for 15-min call]

COMING NEXT:
[Mention any features you're building based on feedback]

Thanks!
Tim
```

---

## 6. Feature Update Email

**Subject:** New Feature: [Feature Name]

```
Hi [Name],

Based on your feedback, we just shipped something new!

WHAT'S NEW:
[Feature name] - [One sentence description]

WHY WE BUILT THIS:
You and [X other] beta users mentioned [problem].

HOW TO USE IT:
1. [Step 1]
2. [Step 2]
3. [Step 3]

TRY IT OUT:
It's live in your account right now - no update needed.

Let me know what you think!

Best,
Tim

P.S. This shipped because of YOUR feedback. Keep it coming!
```

---

## 7. NPS Survey Email (End of Month 1)

**Subject:** Quick feedback request (1 question)

```
Hi [Name],

You've been using Pipeline 360 UVIP for a month now - thank you!

One quick question:

On a scale of 0-10, how likely are you to recommend Pipeline 360 UVIP to a colleague?

Just reply with a number (0-10).

Bonus points if you tell me why!

Thanks,
Tim
```

---

## 8. Beta End/Conversion Email

**Subject:** Beta Update - What's Next

```
Hi [Name],

First, thank you. Your feedback over the past [X weeks] has been incredible.

Here's what we built because of you:
- [Feature 1]
- [Feature 2]
- [Feature 3]

WHAT'S NEXT:

We're transitioning from beta to paid plans on [date].

AS A BETA USER, YOU GET:
- [X]% lifetime discount
- [Grandfathered features]
- Priority support forever

YOUR OPTIONS:
1. Continue at $[X]/month (normally $[Y]) - Reply "YES"
2. Schedule a call to discuss - [Calendar link]
3. End your access - Just let me know

No pressure either way - your feedback has already been invaluable.

Thanks for being part of this journey!

Tim
```

---

## LinkedIn Announcement Post

```
Exciting news for mortgage professionals!

We're launching the Pipeline 360 UVIP beta program - the first AI-powered video intelligence platform built specifically for mortgage.

What you get:
- Automatic call recording & AI analysis
- Personalized video clips for borrowers
- AI coaching to improve every conversation
- Team performance dashboard

Beta benefits:
- Free during entire beta period
- Lifetime discount when we launch
- Direct influence on product development

Limited to 10 teams. Apply today:
https://mortgage-crm-production-7a9a.up.railway.app/beta

#mortgage #proptech #AI #sales
```

---

## Email to Existing Contacts

**Subject:** You're invited: Pipeline 360 UVIP Beta

```
Hi [Name],

I wanted to give you first access to something we've been building.

Pipeline 360 UVIP is the first AI-powered video intelligence platform for mortgage professionals. It automatically:

- Records and transcribes every call
- Provides AI coaching on your conversations
- Lets you send personalized video clips to borrowers
- Tracks team performance with manager dashboard

We're launching a beta program (limited to 10 teams) and I'd love for you to be part of it.

Beta perks:
- Free during entire beta
- Lifetime discount at launch
- Your feedback shapes the product

Apply here: https://mortgage-crm-production-7a9a.up.railway.app/beta

Or just reply and I'll get you set up directly.

Best,
Tim
```
