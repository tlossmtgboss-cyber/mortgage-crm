...
echo "   URL: https://api.perenniaai.com"
echo "   Status: ✅ Online"
echo ""
...
for endpoint in "${ENDPOINTS[@]}"; do
  if echo "$endpoint" | grep -q "drop"; then
    ...
    RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "https://api.perenniaai.com/api/v1/$endpoint" \
      -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d '{}')
  else
    # GET endpoint
    RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" "https://api.perenniaai.com/api/v1/$endpoint" \
...
echo "      https://api.perenniaai.com/api/v1/webhooks/vapi/voicemail-status"
