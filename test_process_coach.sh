...
LOGIN_RESPONSE=$(curl -s -X POST https://app.perenniaai.com/token \
...
DAILY_BRIEFING_RESPONSE=$(curl -s -X POST https://app.perenniaai.com/api/v1/ai/smart-chat \
...
PIPELINE_AUDIT_RESPONSE=$(curl -s -X POST https://app.perenniaai.com/api/v1/ai/smart-chat \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
PRIORITY_RESPONSE=$(curl -s -X POST https://app.perenniaai.com/api/v1/ai/smart-chat \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
MEMORY_STATS=$(curl -s -X GET https://app.perenniaai.com/api/v1/ai/memory-stats \
  -H "Authorization: Bearer $TOKEN")
