...
TOKEN=$(curl -s -X POST https://api.perenniaai.com/token -d "username=admin@perenniaai.com&password=demo123" | jq -r '.access_token')
...
curl -s -H "Authorization: Bearer $TOKEN" \
  "https://api.perenniaai.com/api/v1/users/1/audit-log?limit=5" | jq '.'
echo ""
...
curl -s -H "Authorization: Bearer $TOKEN" \
  "https://api.perenniaai.com/api/v1/users/1/impersonation-history" | jq '.'
echo ""
...
curl -s -H "Authorization: Bearer $TOKEN" \
  "https://api.perenniaai.com/api/v1/users/1/active-sessions" | jq '.'
echo ""
...
curl -s -H "Authorization: Bearer $TOKEN" \
  "https://api.perenniaai.com/api/v1/users/1/audit-log?start_date=2025-11-01&limit=5" | jq '.'
echo ""
