# Client File Routes Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the backend API routes that serve the frontend `client-file/` module — making the `/clients/:uuid` page functional.

**Architecture:** Single route file `routes/client_file_routes.py` registered via `inline_legacy_routes.py` at prefix `/api/v1`. Follows the `team_chat_routes.py` pattern: `APIRouter`, lazy auth import via `get_current_user_dep()`, `get_db` from `db.py`. Endpoints serve data from the `client_files` table directly for CRUD, and join through `client_files.lead_id` to query `tasks` and `followup_cadences`. Features without backing models (timeline, insights, documents, relationships, action plans) return typed empty responses so the frontend renders empty states instead of 404 errors.

**Tech Stack:** FastAPI, SQLAlchemy 2.0, Pydantic v2, PostgreSQL (RLS via `get_db`)
...
| File | Responsibility |
|------|----------------|
| **Create:** `backend/routes/client_file_routes.py` | All `/clients/{id}/*` endpoints — CRUD, tasks, cadence, stubs |
| **Modify:** `backend/routes/inline_legacy_routes.py` | Register the new router |

---

### Task 1: Core Client File CRUD — GET and PATCH

**Files:**
- Create: `backend/routes/client_file_routes.py`

- [ ] **Step 1: Create the route file with GET /clients/{id}**
...
Run: `cd backend && python3 -c "import py_compile; py_compile.compile('routes/client_file_routes.py', doraise=True); print('OK')"`
Expected: `OK`

- [ ] **Step 4: Commit**

```bash
git add backend/routes/client_file_routes.py
git commit -m "feat: add client file GET and PATCH routes"
```

---

### Task 2: Lifecycle, Sticky Note, Tags endpoints

**Files:**
- Modify: `backend/routes/client_file_routes.py`

- [ ] **Step 1: Add lifecycle, sticky-note, and tag endpoints**

Append to `client_file_routes.py`:
...
```bash
git add backend/routes/client_file_routes.py
git commit -m "feat: add lifecycle, sticky-note, and tag endpoints for client file"
```

---

### Task 3: Tasks sub-resource (read from existing `tasks` table)

**Files:**
- Modify: `backend/routes/client_file_routes.py`

- [ ] **Step 1: Add task list, create, complete, reopen endpoints**
...
```bash
git add backend/routes/client_file_routes.py
git commit -m "feat: add task CRUD endpoints for client file"
```

---

### Task 4: Cadence sequences sub-resource

**Files:**
- Modify: `backend/routes/client_file_routes.py`

- [ ] **Step 1: Add cadence-sequences endpoint**

Append to `client_file_routes.py`:

```python
# ─── Cadence sequences (projected from followup_cadences via lead_id) ────
...
def list_cadence_sequences(
    ...
    if not lead:
        return []
    # Find loan_ids for this lead to query executions
...
```bash
git add backend/routes/client_file_routes.py
git commit -m "feat: add cadence-sequences endpoint for client file"
```

---

### Task 5: Stub endpoints for unbuilt features

**Files:**
- Modify: `backend/routes/client_file_routes.py`

- [ ] **Step 1: Add typed empty-response stubs**

Append to `client_file_routes.py`:

```python
# ─── Stubs — return typed empty responses for unbuilt features ───────────
# These prevent 404s so the frontend renders empty-state UI instead of errors.
...
```bash
git add backend/routes/client_file_routes.py
git commit -m "feat: add stub endpoints for timeline, insight, documents, relationships"
```

---

### Task 6: Register the router in inline_legacy_routes.py

**Files:**
- Modify: `backend/routes/inline_legacy_routes.py:1074`

- [ ] **Step 1: Add router registration after team chat routes block**

Insert the following block after the team chat routes try/except block (after line 1074):
...
Run: `cd backend && python3 -c "import py_compile; py_compile.compile('routes/inline_legacy_routes.py', doraise=True); print('OK')"`
Expected: `OK`

- [ ] **Step 3: Commit**

```bash
git add backend/routes/inline_legacy_routes.py
git commit -m "feat: register client file routes in inline_legacy_routes"
```
...
Monitor the mortgage-crm service deploy until SUCCESS.

- [ ] **Step 3: Smoke test the API**

```bash
# Should return 401 (auth required) not 404
curl -s -o /dev/null -w "%{http_code}" https://api.perenniaai.com/api/v1/clients/00000000-0000-0000-0000-000000000000
# Expected: 401 or 403

# Stub endpoints should also not 404
curl -s -o /dev/null -w "%{http_code}" https://api.perenniaai.com/api/v1/clients/00000000-0000-0000-0000-000000000000/timeline
curl -s -o /dev/null -w "%{http_code}" https://api.perenniaai.com/api/v1/clients/00000000-0000-0000-0000-000000000000/tasks
# Expected: 401 or 403 for all (not 404)
```
