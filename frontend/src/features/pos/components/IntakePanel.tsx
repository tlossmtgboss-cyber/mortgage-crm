...
export const IntakePanel: React.FC<IntakePanelProps> = ({ initial, onComplete }) => {
  const [step, setStep] = useState<'veteran' | 'coborrower' | 'names'>(
    initial?.is_veteran != null
      ? initial?.has_co_borrower != null
        ? 'names'
        : 'coborrower'
      : 'veteran',
  );
  ...
  const handleCoBorrowerSelect = (val: boolean) => {
    ...
    if (val) {
      ...
      onComplete({ is_veteran: isVeteran, has_co_borrower: false, co_borrowers: [] });
    }
  };
  ...
  const handleNamesSubmit = () => {
    ...
    onComplete({ is_veteran: isVeteran, has_co_borrower: true, co_borrowers: valid });
  };
    ...
    <div className="pos-intake">
      ...
      {step === 'veteran' && (
        <div className="pos-intake__card" key="veteran">
          ...
          <div className="pos-intake__choices">
            <button
              type="button"
              className={`pos-intake__choice${isVeteran === true ? ' is-selected' : ''}`}
            ...
            <button
              type="button"
              className={`pos-intake__choice${isVeteran === false ? ' is-selected' : ''}`}
      ...
      {step === 'coborrower' && (
        <div className="pos-intake__card" key="coborrower">
          <button type="button" className="pos-intake__back" onClick={() => setStep('veteran')}>
          ...
          <div className="pos-intake__choices">
            <button
              type="button"
              className={`pos-intake__choice${hasCoBorrower === true ? ' is-selected' : ''}`}
            ...
            <button
              type="button"
              className={`pos-intake__choice${hasCoBorrower === false ? ' is-selected' : ''}`}
      ...
      {step === 'names' && (
        <div className="pos-intake__card" key="names">
          <button type="button" className="pos-intake__back" onClick={() => setStep('coborrower')}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <polyline points="15 18 9 12 15 6" />
            </svg>
            Back
            ...
            {coBorrowers.map((cb, idx) => (
              <div key={idx} className="pos-intake__name-row">
                ...
                <div className="pos-intake__name-fields">
                  <input
                    type="text"
                    className="pos-intake__input"
                    placeholder="First name"
                    value={cb.first_name}
                    onChange={e => updateCoBorrower(idx, 'first_name', e.target.value)}
                  />
                  <input
                    type="text"
                    className="pos-intake__input"
                    ...
                    value={cb.last_name}
                    onChange={e => updateCoBorrower(idx, 'last_name', e.target.value)}
                  />
