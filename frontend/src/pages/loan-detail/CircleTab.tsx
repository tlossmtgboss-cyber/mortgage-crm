...
import type { CircleContact, CircleContactType, CircleForm, SearchResult } from './types';

interface CircleTabProps {
  cashflowLoading: boolean;
  cashflowOpportunities: any[];
  cashflowReferrals: any[];
  circleContacts: CircleContact[];
  showCircleModal: boolean;
  setShowCircleModal: (show: boolean) => void;
  circleForm: CircleForm;
  setCircleForm: (form: CircleForm) => void;
  circleContactTypes: CircleContactType[];
  searchResults: SearchResult[];
  searchLoading: boolean;
  showSearchResults: boolean;
  setShowSearchResults: (show: boolean) => void;
  handleNameChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
  selectSearchResult: (contact: SearchResult) => void;
  handleAddCircleContactSubmit: () => void;
  handleDeleteCircleContact: (contactId: number) => void;
  handleEditCircleContact: (contact: CircleContact) => void;
  getContactIcon: (type: string) => string;
  searchContacts: (query: string, contactType?: string) => void;
}
...
export default function CircleTab({
  ...
  handleNameChange,
  selectSearchResult,
  handleAddCircleContactSubmit,
  handleDeleteCircleContact,
  handleEditCircleContact,
  getContactIcon,
  searchContacts,
}: CircleTabProps) {
    ...
    <div className="info-section">
      ...
      <div className="circle-content">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
          ...
          <button className="btn-add-circle" onClick={() => setShowCircleModal(true)} style={{ padding: '8px 16px' }}>
            + Add Contact
          </button>
        </div>
            ...
            circleContacts.map(contact => (
              <div key={contact.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 12px', backgroundColor: '#f8f9fa', borderRadius: '6px', border: '1px solid #e9ecef' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      {contact.loanId ? (
                        <span onClick={() => navigate(`/loans/${contact.loanId}`)}
                ...
                <div style={{ display: 'flex', gap: '4px' }}>
                  <button onClick={() => handleEditCircleContact(contact)} style={{ background: 'none', border: 'none', color: '#217f8d', cursor: 'pointer', fontSize: '14px', padding: '4px 8px' }} title="Edit contact">
                    ✏️
                  </button>
                  <button onClick={() => handleDeleteCircleContact(contact.id)} style={{ background: 'none', border: 'none', color: '#dc3545', cursor: 'pointer', fontSize: '16px', padding: '4px 8px' }} title="Remove contact">
                    ×
                  </button>
                </div>
              </div>
            ...
            <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '500px' }}>
              ...
              <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div className="form-group">
                  ...
                  <select value={circleForm.type} onChange={e => {
                    const newType = e.target.value;
                    setCircleForm({...circleForm, type: newType});
                    if (circleForm.name.length >= 2) { searchContacts(circleForm.name, newType); }
                  }} className="form-control">
                    {circleContactTypes.map(type => (<option key={type.value} value={type.value}>{type.value}</option>))}
                  </select>
                </div>
                <div className="form-group" style={{ position: 'relative' }}>
                  <label>Name *</label>
                  <input type="text" value={circleForm.name} onChange={handleNameChange}
                    onFocus={() => circleForm.name.length >= 2 && setShowSearchResults(true)}
                    className="form-control" placeholder="Start typing to search..." autoComplete="off" />
                  {searchLoading && (
                  ...
                  {showSearchResults && searchResults.length > 0 && (
                    <div style={{
                      ...
                      {searchResults.map(result => (
                        <div key={result.id} onClick={() => selectSearchResult(result)}
                          ...
                          <div style={{ fontSize: '12px', color: '#666' }}>
                            ...
                            {(result.borrower_email || result.email) && (result.borrower_phone || result.phone) && <span> &bull; </span>}
              ...
              <div className="modal-footer">
                ...
                <button className="btn-primary" onClick={handleAddCircleContactSubmit} disabled={!circleForm.name.trim()}>
