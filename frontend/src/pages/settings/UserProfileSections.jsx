                      ...
                      {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((d, di) => {
                        ...
                        return (
                          <button key={d} type="button" onClick={() => {
                            ...
                            setUserProfile({ ...userProfile, blocked_times: updated });
                          }} style={{ padding: '2px 6px', fontSize: '11px', borderRadius: '4px', cursor: 'pointer', border: '1px solid', transition: 'all 0.1s', borderColor: isSelected ? '#217F8D' : '#d1d5db', background: isSelected ? 'rgba(33, 127, 141, 0.1)' : 'white', color: isSelected ? '#217F8D' : '#999', fontWeight: isSelected ? '600' : '400' }}>{d}</button>
                        );
                      })}
                    </div>
