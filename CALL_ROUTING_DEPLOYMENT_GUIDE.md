...
curl -X POST https://api.perenniaai.com/api/vapi/receptionist/staff-availability \
...
curl -X POST https://api.perenniaai.com/api/vapi/functions/identify-caller \
  -H "Content-Type: application/json" \
...
**Steps:**
```bash
curl -X GET "https://api.perenniaai.com/api/vapi/receptionist/available-staff" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
**Steps:**
```bash
curl -X GET "https://api.perenniaai.com/api/vapi/receptionist/routing-log?limit=10" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
# Production Domain
PRODUCTION_DOMAIN=https://api.perenniaai.com
```
...
```bash
# Via API
curl -X GET "https://api.perenniaai.com/api/vapi/receptionist/routing-log" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
```bash
curl -X GET "https://api.perenniaai.com/api/vapi/receptionist/available-staff?role=production_assistant" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
2. Check routing logs for error details:
   ```bash
   curl -X GET "https://api.perenniaai.com/api/vapi/receptionist/routing-log?limit=1" \
     -H "Authorization: Bearer YOUR_JWT_TOKEN"
   ```
