...
```bash
# Login to get token
curl -X POST https://api.perenniaai.com/api/v1/login \
...
```bash
curl -X GET https://api.perenniaai.com/api/v1/merge/duplicates \
...
```bash
curl -X POST https://api.perenniaai.com/api/v1/merge/execute \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "pair_id": 1,
