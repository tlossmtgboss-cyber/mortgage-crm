...
```bash
curl -X POST 'https://app.perenniaai.com/api/ai/events?event_type=LeadCreated' \
...
```bash
curl -X POST 'https://app.perenniaai.com/api/ai/events?event_type=DailyPipelineSweep' \
  -H 'Content-Type: application/json' \
  -d '{
    "entity_type": "system",
...
```bash
curl -X POST 'https://app.perenniaai.com/api/ai/events?event_type=MonthlyPortfolioReview' \
  -H 'Content-Type: application/json' \
  -d '{
...
curl 'https://app.perenniaai.com/api/ai/executions?agent_id=lead_manager'

# See Pipeline Manager's recent activity
curl 'https://app.perenniaai.com/api/ai/executions?agent_id=pipeline_manager&limit=10'
```
...
curl 'https://app.perenniaai.com/api/ai/agents/lead_manager/metrics?days=30'
```
...
```bash
curl 'https://app.perenniaai.com/api/ai/executions?hours=1'
```
