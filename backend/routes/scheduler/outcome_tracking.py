...
Endpoints:
  ...
  - GET /analytics/lo-outcome-comparison    Per-LO appointment outcome breakdown
  - GET /analytics/outcome-timeline         Time-series of appointment outcomes
...
class LOOutcomeEntry(BaseModel):
    """Per-LO outcome metrics."""
    user_id: Optional[int] = Field(None)
    lo_name: str = Field("")
    total_appointments: int = Field(0)
    ai_scheduled: int = Field(0)
    manually_scheduled: int = Field(0)
    linked_to_loan: int = Field(0)
    funded_count: int = Field(0)
    funded_volume: float = Field(0.0)
    funded_volume_formatted: str = Field("$0.00")
    conversion_rate: float = Field(0.0)
    avg_days_to_fund: Optional[float] = Field(None)
    no_show_count: int = Field(0)
    no_show_rate: float = Field(0.0)


class LOOutcomeResponse(BaseModel):
    """LO outcome comparison response."""
    period_days: int = Field(90)
    loan_officers: List[LOOutcomeEntry] = Field(default_factory=list)

    class Config:
        from_attributes = True


class OutcomeTimelineEntry(BaseModel):
    """Single time bucket in the outcome timeline."""
    period_label: str = Field("", description="Date label (e.g. '2026-03' or '2026-W10')")
    total_appointments: int = Field(0)
    funded_count: int = Field(0)
    funded_volume: float = Field(0.0)
    conversion_rate: float = Field(0.0)
    ai_funded: int = Field(0)
    manual_funded: int = Field(0)


class OutcomeTimelineResponse(BaseModel):
    """Time-series outcome data."""
    period_days: int = Field(90)
    granularity: str = Field("monthly")
    entries: List[OutcomeTimelineEntry] = Field(default_factory=list)
...
# =============================================================================
# PER-LO OUTCOME COMPARISON
# =============================================================================

@router.get("/analytics/lo-outcome-comparison", response_model=LOOutcomeResponse)
@require_feature_tier("scheduler_analytics")
async def get_lo_outcome_comparison(
    request: Request,
    days: int = Query(90, ge=7, le=365, description="Lookback period in days"),
    db: Session = Depends(get_db),
):
    """
    Get per-loan-officer appointment outcome breakdown.

    Admin-only endpoint showing which LOs convert appointments to funded
    loans most effectively, with AI vs manual scheduling split.
    """
    current_user = await get_current_user(request, db)
    org_id = _get_org_id(current_user)
    _resolve_effective_user_id(current_user, None, require_admin_for_all=True)
    ...
    try:
        rows = db.execute(text("""
            SELECT
                ...
                COALESCE(CONCAT(u.first_name, ' ', u.last_name), 'Unassigned') as lo_name,
                COUNT(*) as total_appointments,
                COUNT(CASE WHEN a.booked_by_ai = true THEN 1 END) as ai_scheduled,
                COUNT(CASE WHEN a.booked_by_ai = false OR a.booked_by_ai IS NULL THEN 1 END) as manually_scheduled,
                COUNT(a.loan_id) as linked_to_loan,
                COUNT(CASE WHEN l.stage = 'FUNDED' THEN 1 END) as funded_count,
                COALESCE(SUM(CASE WHEN l.stage = 'FUNDED' THEN l.amount END), 0) as funded_volume,
                AVG(CASE WHEN l.funded_date IS NOT NULL
            ...
            WHERE a.organization_id = :org_id
                AND a.scheduled_start >= CURRENT_DATE - :days
            GROUP BY a.assigned_user_id, u.first_name, u.last_name
            ORDER BY funded_count DESC, total_appointments DESC
        """), params).fetchall()
        ...
        for row in rows:
            ...
            linked = int(m["linked_to_loan"] or 0)
            funded = int(m["funded_count"] or 0)
            total = int(m["total_appointments"] or 0)
            volume = float(m["funded_volume"] or 0)
            avg_dtf = float(m["avg_days_to_fund"]) if m["avg_days_to_fund"] is not None else None
            no_shows = int(m["no_show_count"] or 0)

            loan_officers.append(LOOutcomeEntry(
                user_id=m["user_id"],
                lo_name=m["lo_name"],
                total_appointments=total,
                ai_scheduled=int(m["ai_scheduled"] or 0),
                manually_scheduled=int(m["manually_scheduled"] or 0),
                linked_to_loan=linked,
                funded_count=funded,
                funded_volume=volume,
                funded_volume_formatted=_format_currency(volume),
                ...
                no_show_rate=_calc_rate(no_shows, total),
            ))

        return LOOutcomeResponse(
            period_days=days,
            loan_officers=loan_officers,
        )
    except Exception as e:
        logger.exception(f"LO outcome comparison query failed: {e}")
        raise HTTPException(status_code=500, detail="Failed to compute LO outcome comparison")


# =============================================================================
# OUTCOME TIMELINE (time-series)
# =============================================================================

@router.get("/analytics/outcome-timeline", response_model=OutcomeTimelineResponse)
@require_feature_tier("scheduler_analytics")
async def get_outcome_timeline(
    request: Request,
    days: int = Query(90, ge=7, le=365, description="Lookback period in days"),
    granularity: str = Query("monthly", regex="^(weekly|monthly)$", description="Time bucket granularity"),
    user_id: Optional[int] = Query(None, description="Filter by specific LO (admin only)"),
    db: Session = Depends(get_db),
):
    """
    Time-series of appointment outcomes for trend charts.

    Shows how appointment-to-funding conversion rates change over time,
    with AI vs manual breakdown per period.
    """
    current_user = await get_current_user(request, db)
    org_id = _get_org_id(current_user)
    effective_user_id = _resolve_effective_user_id(current_user, user_id)
    ...
    if granularity == "weekly":
        trunc_expr = "TO_CHAR(DATE_TRUNC('week', a.scheduled_start), 'IYYY-\"W\"IW')"
    else:
        trunc_expr = "TO_CHAR(DATE_TRUNC('month', a.scheduled_start), 'YYYY-MM')"

    try:
        rows = db.execute(text(f"""
            SELECT
                {trunc_expr} as period_label,
                COUNT(*) as total_appointments,
                COUNT(CASE WHEN l.stage = 'FUNDED' THEN 1 END) as funded_count,
                COALESCE(SUM(CASE WHEN l.stage = 'FUNDED' THEN l.amount END), 0) as funded_volume,
                COUNT(CASE WHEN l.stage = 'FUNDED' AND a.booked_by_ai = true THEN 1 END) as ai_funded,
                COUNT(CASE WHEN l.stage = 'FUNDED' AND (a.booked_by_ai = false OR a.booked_by_ai IS NULL) THEN 1 END) as manual_funded
            FROM scheduler_appointments a
            LEFT JOIN loans l ON l.id = a.loan_id
            WHERE a.organization_id = :org_id
                AND a.scheduled_start >= CURRENT_DATE - :days
        ...
        for row in rows:
            ...
            entries.append(OutcomeTimelineEntry(
                period_label=m["period_label"] or "",
                total_appointments=total,
                funded_count=funded,
                funded_volume=volume,
                conversion_rate=_calc_rate(funded, total),
                ai_funded=int(m["ai_funded"] or 0),
                manual_funded=int(m["manual_funded"] or 0),
            ))

        return OutcomeTimelineResponse(
            period_days=days,
            granularity=granularity,
            entries=entries,
        )
    except Exception as e:
        logger.exception(f"Outcome timeline query failed: {e}")
        raise HTTPException(status_code=500, detail="Failed to compute outcome timeline")
