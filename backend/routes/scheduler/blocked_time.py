...
def _audit_log(db, org_id, user_id, action, entity_type, entity_id=None, changes=None, request=None):
    """Delegate to parent's audit log helper."""
    if _helpers and '_audit_log' in _helpers:
        return _helpers['_audit_log'](db, org_id, user_id, action, entity_type,
                                      entity_id=entity_id, changes=changes, request=request)
