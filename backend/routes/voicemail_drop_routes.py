...
# =============================================================================
# TEMPORARY: Admin-key test endpoint for voicemail drop (DELETE after testing)
# =============================================================================

@router.post("/test-drop")
async def test_voicemail_drop(
    request: Request,
    db: Session = Depends(get_db),
):
    """
    Test voicemail drop using admin key auth (no user session required).
    DELETE THIS ENDPOINT after confirming voicemail works.

    Body: { "admin_key": str, "phone_number": str, "message": str, "recipient_name": str }
    """
    data = await request.json()
    ...
    phone_number = data.get("phone_number")
    message_text = data.get("message", "Hi there! This is a test voicemail from the Perennia AI system. Hope you are having an awesome day! Talk to you soon!")
    recipient_name = data.get("recipient_name", "Timothy")
    ...
    try:
        # Create a test voicemail drop record (use user_id=118 for tloss)
        voicemail_drop = VoicemailDrop(
            user_id=118,
            phone_number=phone_number,
            contact_name=recipient_name,
