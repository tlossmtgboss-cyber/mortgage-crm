...
from services.email_tracking_service import PIXEL_GIF, record_open, record_click, get_tracking_stats
...
async def track_open(tracking_id: str, request: Request, db=Depends(get_db)):
...
async def track_click(tracking_id: str, link_hash: str, request: Request, db=Depends(get_db)):
