...
# Intent → scoped tool names (single source of truth for analyze + orchestrator)
INTENT_TO_SCOPED_TOOLS: Dict[str, List[str]] = {
    "greeting": [],
    "simple": ["get_daily_priorities"],
    ...
    "leads": ["lead_status_insights", "get_leads_by_status", "get_top_leads", "get_stale_leads", "search_leads", "find_contact"],
    "top_leads": ["get_top_leads", "score_lead", "suggest_followup", "get_lead_details"],
    "pipeline": ["get_pipeline", "get_pipeline_metrics", "search_loans", "get_loan_file"],
    ...
    "calls": ["click_to_dial", "make_call", "call_contact", "get_top_leads", "search_leads", "find_contact", "find_contact_phone", "find_contact_email"],
    "email": ["get_emails_needing_response", "send_email", "search_email_inbox", "search_leads", "search_loans", "create_referral_partner", "find_contact", "find_contact_email", "find_contact_phone", "send_sms", "send_sms_message", "send_notification"],
    "schedule": ["get_availability", "book_appointment", "reschedule_appointment", "cancel_appointment", "get_upcoming_appointments", "send_appointment_reminder", "sync_external_calendar", "optimize_schedule", "find_contact", "find_contact_phone", "find_contact_email"],
    ...
    "customer": ["get_customer_360", "map_relationships", "calculate_ltv", "assess_churn_risk", "find_opportunities", "get_interaction_history", "get_referral_network", "search_leads", "search_loans", "find_contact", "get_loan_file", "search_mum_clients"],
    ...
    "compound": ["get_pipeline_metrics", "search_loans", "search_leads", "get_tasks", "create_task", "bulk_create_tasks", "send_email", "suggest_followup", "find_contact", "find_contact_phone", "find_contact_email", "send_sms", "send_sms_message", "click_to_dial", "make_call", "call_contact", "send_notification", "get_availability", "book_appointment", "reschedule_appointment", "get_upcoming_appointments", "send_appointment_reminder"],
    "content_marketing": ["get_pipeline_metrics", "search_leads", "draft_message"],
    "general": ["get_daily_priorities", "get_pipeline", "get_tasks", "search_leads", "find_contact"],
}
...
INTENT_PATTERNS: Dict[str, List[str]] = {
    ...
    "leads": [
        ...
        # Name lookup patterns — "who is [Name]", "find [Name]", "look up [Name]"
        r"who is [A-Z][a-z]+",
        r"(find|look up|lookup|search for|pull up) [A-Z][a-z]+",
        r"(do we have|is there) (a )?(lead|contact|borrower) (named|called|for)",
        r"(what|where) is [A-Z][a-z]+ [A-Z][a-z]+'?s? (file|loan|status|info|number|email|phone)",
    ],
    "pipeline": [
    ...
    "customer": [
        ...
        r"referral",
        r"\bmum\b",
        r"member.{0,5}under.{0,5}manage",
        r"past (client|borrower|customer)",
        r"(funded|closed) (client|borrower|loan|file)",
        r"(former|previous|existing) (client|borrower|customer)",
        r"refinance opportunit",
        r"(client|borrower) (portfolio|book|database)",
    ],
    "integrations": [
        r"integrat",
        r"\bLOS\b",
