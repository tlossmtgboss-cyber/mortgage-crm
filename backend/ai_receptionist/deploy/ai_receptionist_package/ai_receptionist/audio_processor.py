...
class AudioProcessor:
    ...
    1. Receive audio from Telnyx (mu-law)
    2. Convert to format for STT
    3. Get transcription
    4. Process through conversation engine
    5. Synthesize response
    6. Convert back to mu-law for Telnyx
    """
    ...
    async def transcribe_telephony_audio(self, mulaw_audio: bytes) -> str:
        """
        Transcribe audio from telephony provider (mu-law format).

        Args:
            mulaw_audio: Mu-law encoded audio from Telnyx
    ...
    async def synthesize_for_telephony(self, text: str) -> bytes:
        ...
        Returns:
            Mu-law encoded audio for Telnyx
        """
    ...
    async def process_realtime(
        ...
        Args:
            audio_stream: Incoming audio chunks (mu-law from Telnyx)
...
async def transcribe_audio(
    ...
    async with create_audio_processor(stt_provider=provider) as processor:
        if format == AudioFormat.MULAW:
            return await processor.transcribe_telephony_audio(audio_data)
