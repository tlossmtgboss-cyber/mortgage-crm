...
Creates infrastructure for multi-tenant microsite system with:
- Template packs with versioning
- User microsites with customizable content
- Analytics and lead tracking
- Organization-level compliance settings

The platform provides:
- Schema-driven content editing
- Template version pinning for stability
- Compliance-locked fields at org level
- Full analytics and lead capture
"""
import sys
sys.path.append('..')
...
def run_migration():
    """Create Microsite Platform tables"""
    ...
    sql_commands = [
        ...
        """
        CREATE INDEX IF NOT EXISTS idx_microsites_published
        ON microsites(status, published_at) WHERE status = 'published';
        """,
        ...
        """
        CREATE INDEX IF NOT EXISTS idx_publish_history_site
        ON microsite_publish_history(microsite_id, published_at DESC);
        """,
        ...
        """
        CREATE INDEX IF NOT EXISTS idx_analytics_events_site_date
        ON microsite_analytics_events(microsite_id, created_at DESC);
        """,

        """
        CREATE INDEX IF NOT EXISTS idx_analytics_events_type
        ON microsite_analytics_events(event_type, created_at DESC);
        """,

        """
        CREATE INDEX IF NOT EXISTS idx_analytics_events_session
        ON microsite_analytics_events(session_id);
        """,
        ...
        """
        CREATE INDEX IF NOT EXISTS idx_microsite_leads_site
        ON microsite_leads(microsite_id, created_at DESC);
        """,

        """
        CREATE INDEX IF NOT EXISTS idx_microsite_leads_user
        ON microsite_leads(user_id, status);
        """,
        ...
        """
        CREATE INDEX IF NOT EXISTS idx_microsite_leads_status
        ON microsite_leads(status, created_at DESC);
        """,
        ...
        # ============================================================================
        """
        CREATE INDEX IF NOT EXISTS idx_microsites_content_gin
        ON microsites USING GIN (content_json);
        """,

        """
        CREATE INDEX IF NOT EXISTS idx_template_packs_schema_gin
        ON microsite_template_packs USING GIN (schema_json);
        """,
    ...
    logger.info("Microsite Platform migration completed successfully!")


def rollback_migration():
    """Drop all Microsite Platform tables (use with caution!)"""

    drop_commands = [
        "DROP TABLE IF EXISTS microsite_analytics_events CASCADE;",
        "DROP TABLE IF EXISTS microsite_leads CASCADE;",
        "DROP TABLE IF EXISTS microsite_publish_history CASCADE;",
        "DROP TABLE IF EXISTS microsite_assets CASCADE;",
        "DROP TABLE IF EXISTS microsites CASCADE;",
        "DROP TABLE IF EXISTS microsite_template_packs CASCADE;",
        "DROP TABLE IF EXISTS organization_microsite_settings CASCADE;",
        "DROP FUNCTION IF EXISTS microsite_update_updated_at_column() CASCADE;",
        "DROP FUNCTION IF EXISTS record_microsite_publish_event() CASCADE;",
    ]
    ...
    logger.info("Microsite Platform tables dropped!")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "rollback":
        confirm = input("Are you sure you want to drop all Microsite tables? (yes/no): ")
