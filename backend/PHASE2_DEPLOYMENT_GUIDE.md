...
```bash
# Get auth token
TOKEN=$(curl -s -X POST "https://app.perenniaai.com/api/v1/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"username": "admin@perenniaai.com", "password": "password"}' | jq -r '.access_token')

# Test AI query
curl -X POST "https://app.perenniaai.com/api/v1/profitability/ai/query" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"question": "What is our cost per loan?"}'

# Test recommendations
curl "https://app.perenniaai.com/api/v1/profitability/ai/recommendations" \
  -H "Authorization: Bearer $TOKEN"

# Test suggested questions
curl "https://app.perenniaai.com/api/v1/profitability/ai/suggested-questions" \
  -H "Authorization: Bearer $TOKEN"
```
