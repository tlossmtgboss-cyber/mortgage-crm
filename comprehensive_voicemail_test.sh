...
echo "--------------------------------------"
HISTORY_RESPONSE=$(curl -s "https://app.perenniaai.com/api/v1/voicemail/history?limit=5" \
  -H "Authorization: Bearer $TOKEN")
...
echo "--------------------------------------------------"
TRANSCRIBE_RESPONSE=$(curl -s -X POST "https://app.perenniaai.com/api/v1/voicemail/transcribe" \
  -H "Authorization: Bearer $TOKEN")
