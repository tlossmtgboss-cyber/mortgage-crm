...
RESPONSE=$(curl -s -X POST "https://api.perenniaai.com/api/v1/admin/sql-debug" \
...
MIGRATION_RESPONSE=$(curl -s -X POST "https://api.perenniaai.com/api/v1/migrations/add-voicemail-system" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json")
